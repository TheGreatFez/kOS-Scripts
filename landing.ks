...
print "Aligning with Orbit Retrograde".
wait 15.
...
until flightmode = 2 AND ship:status = "LANDED" {
	...
	if flightmode = 1 {
		...
		set ThrustSet to 1 - Kp_v*errorP_v - Kd_v*errorD_v.
	...
	print "Error Vert = " + round(error_v,2) + "     " at(0,6).
	...
	//log round(time1,3) +","+ round(Vmax,2) +","+ -1*round(verticalspeed,2) +","+ round(true_alt,2) +","+ round(error_v,2) +","+ 100*round(ThrustSet,2) +","+ round(MaxAccUp,2) to landingdata.csv.
